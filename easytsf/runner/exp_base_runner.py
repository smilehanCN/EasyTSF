...
class LTSFRunner(L.LightningModule):
    ...
    def test_step(self, batch, batch_idx):
        ...
        # wape = weighted_absolute_percentage_error(label, prediction)
        # raw_mae = torch.nn.functional.l1_loss(raw_prediction, raw_label)
        # raw_mse = torch.nn.functional.mse_loss(raw_prediction, raw_label)
        # raw_mape = mean_absolute_percentage_error(raw_label, raw_prediction)
        # raw_wape = weighted_absolute_percentage_error(raw_label, raw_prediction)
        ...
        # self.log('test/mape', mape, on_step=False, on_epoch=True, sync_dist=True)
        # self.log('test/wape', wape, on_step=False, on_epoch=True, sync_dist=True)
        # self.log('test/raw_mae', raw_mae, on_step=False, on_epoch=True, sync_dist=True)
        # self.log('test/raw_mse', raw_mse, on_step=False, on_epoch=True, sync_dist=True)
        # self.log('test/raw_mape', raw_mape, on_step=False, on_epoch=True, sync_dist=True)
        # self.log('test/raw_wape', raw_wape, on_step=False, on_epoch=True, sync_dist=True)
