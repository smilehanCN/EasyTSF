...
class LTSFMyNetV6Runner(L.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        rec_mse_loss, rec_mae_loss, rec_patch_loss, rec_cycle_loss, rec_overall_loss = self.model.loss_function(pred_x, var_x)

        pred_loss = self.model.weight_mse * pred_mse_loss + self.model.weight_mae * pred_mae_loss + self.model.weight_patch * pred_patch_loss + self.model.weight_cycle * pred_cycle_loss + self.model.weight_overall * pred_overall_loss
        rec_loss = self.model.weight_mse * rec_mse_loss + self.model.weight_mae * rec_mae_loss + self.model.weight_patch * rec_patch_loss + self.model.weight_cycle * rec_cycle_loss + self.model.weight_overall * rec_overall_loss
        ...
        # self.log('train/pred_mse', pred_mse_loss, on_step=False, on_epoch=True, prog_bar=True, sync_dist=True)
        # self.log('train/pred_p', pred_patch_loss, on_step=False, on_epoch=True, prog_bar=True, sync_dist=True)
        # self.log('train/pred_c', pred_cycle_loss, on_step=False, on_epoch=True, prog_bar=True, sync_dist=True)
        # self.log('train/pred_o', pred_overall_loss, on_step=False, on_epoch=True, prog_bar=True, sync_dist=True)
        return loss
