...
try:
    ...
    XFORMERS_IMPORT_ERROR = exc
else:
    XFORMERS_IMPORT_ERROR = None


def require_xformers() -> None:
    if XFORMERS_IMPORT_ERROR is not None:
        raise ImportError(
            "ARROW requires the optional dependency 'xformers'. Install it before instantiating the ARROW model."
        ) from XFORMERS_IMPORT_ERROR
...
def compute_mixed_cis(freqs: torch.Tensor, t_x: torch.Tensor, t_y: torch.Tensor, num_heads: int):
    num_tokens = t_x.shape[0]
    depth = freqs.shape[1]
    with torch.cuda.amp.autocast(enabled=False):
        freqs_x = (t_x.unsqueeze(-1) @ freqs[0].unsqueeze(-2)).view(depth, num_tokens, num_heads, -1).permute(0, 2, 1, 3)
        freqs_y = (t_y.unsqueeze(-1) @ freqs[1].unsqueeze(-2)).view(depth, num_tokens, num_heads, -1).permute(0, 2, 1, 3)
...
def compute_mixed_cis_optimized(freqs: torch.Tensor, t_x: torch.Tensor, t_y: torch.Tensor, num_heads: int):
    if len(freqs.shape) == 3:
        ...
        depth = freqs.shape[2]
        with torch.cuda.amp.autocast(enabled=False):
            freqs_x = torch.einsum('n,bdhc->bdhnc', t_x, freqs[:, 0].view(batch_size, depth, num_heads, -1))
            freqs_y = torch.einsum('n,bdhc->bdhnc', t_y, freqs[:, 1].view(batch_size, depth, num_heads, -1))
            freqs_cis = torch.polar(torch.ones_like(freqs_x), freqs_x + freqs_y)
    else:
        raise ValueError("Unsupported freqs shape: {}".format(tuple(freqs.shape)))
    return freqs_cis
...
def reshape_for_broadcast(freqs_cis: torch.Tensor, x: torch.Tensor):
    ...
    if freqs_cis.shape == (x.shape[-2], x.shape[-1]):
        ...
        raise ValueError("freqs_cis shape {} is incompatible with {}".format(tuple(freqs_cis.shape), tuple(x.shape)))
...
class MemEffMLA(nn.Module):
    ...
    def forward(self, x, attn_bias=None, freqs_cis=None):
        ...
        if self.q_lora_rank == 0:
            q = self.wq(x).reshape(batch_size, seq_len, self.num_heads, self.head_dim).permute(0, 2, 1, 3)
        else:
            q = self.wq_b(self.q_norm(self.wq_a(x))).reshape(batch_size, seq_len, self.num_heads, self.head_dim).permute(0, 2, 1, 3)
        kv = self.wkv_a(x)
        kv = self.kv_norm(kv)
        kv = self.wkv_b(kv).reshape(batch_size, seq_len, self.num_heads, 2 * self.head_dim)
