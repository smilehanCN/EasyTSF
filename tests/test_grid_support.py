...
class GridSupportTestCase(unittest.TestCase):
    ...
    def _write_meta(self, dataset_dir, dataset_name, split_lengths, freq=60, extra_meta=None):
        meta = {
            "name": dataset_name,
    ...
    def test_grid_data_interface_validates_3d_coord_shape(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            variable = np.arange(15 * 2 * 2 * 3 * 4, dtype=np.float32).reshape(15, 2, 2, 3, 4)
    ...
    def test_grid_data_interface_rejects_legacy_3d_npz_layout(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            dataset_dir = self._make_dataset_dir(tmpdir, "grid3d_legacy")
            variable = np.arange(15 * 2 * 2 * 3 * 4, dtype=np.float32).reshape(15, 2, 2, 3, 4)
            np.savez(dataset_dir / "data.npz", scaled_variable=variable, timestamp=_make_timestamp(variable.shape[0]))
            self._write_meta(dataset_dir, "grid3d_legacy", split_lengths=(5, 5, 5), freq=60)

            with self.assertRaisesRegex(ValueError, "must use storage_format"):
                self._build_grid_datamodule(tmpdir, "grid3d_legacy")

    def test_grid_data_interface_requires_3d_manifest(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            variable = np.arange(15 * 2 * 2 * 3 * 4, dtype=np.float32).reshape(15, 2, 2, 3, 4)
            dataset_dir = self._write_grid3d_sharded_dataset(
                tmpdir,
                "grid3d_missing_manifest",
                variable,
                split_lengths=(5, 5, 5),
            )
            (dataset_dir / "train" / "manifest.json").unlink()

            with self.assertRaisesRegex(FileNotFoundError, "manifest.json"):
                self._build_grid_datamodule(tmpdir, "grid3d_missing_manifest")

    def test_grid_data_interface_reads_cross_shard_3d_window(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            variable = np.arange(22 * 2 * 2 * 3 * 4, dtype=np.float32).reshape(22, 2, 2, 3, 4)
            datamodule_dir = self._write_grid3d_sharded_dataset(
                tmpdir,
                "grid3d_cross_shard",
                variable,
                split_lengths=(12, 5, 5),
            )
            ...
            datamodule = GridDataInterface(
                ...
                precompute_window_index=False,
                data_root=tmpdir,
                dataset_name="grid3d_cross_shard",
            )
            var_x, _, var_y, _ = datamodule.split_datasets["train"][3]

            expected_window = variable[3:8]
            np.testing.assert_allclose(var_x, expected_window[:3])
            np.testing.assert_allclose(var_y, expected_window[3:])
            self.assertTrue((datamodule_dir / "train" / "000000.npy").exists())
