...
log_dir="${repo_dir}/logs/grid3d_risk_v1_0414_dji_inspection_4cls"
mkdir -p "${log_dir}"
run_id="$(date +%Y%m%d_%H%M%S)"

configs=(
  "config/experiments/unet3d/windfield4cast_risk_v1_0414_dji_inspection_4cls.yaml"
  "config/experiments/unet3d_patchcat/windfield4cast_risk_v1_0414_dji_inspection_4cls.yaml"
  "config/experiments/patchstg_flat3d/windfield4cast_risk_v1_0414_dji_inspection_4cls.yaml"
  "config/experiments/fredn_multivariate3d/windfield4cast_risk_v1_0414_dji_inspection_4cls.yaml"
)

for config in "${configs[@]}"; do
  model_dir="$(basename "$(dirname "${config}")")"
  log_file="${log_dir}/${run_id}_${model_dir}.log"
  exp_dir="${repo_dir}/checkpoint/grid3d_risk_v1_0414_dji_inspection_4cls_runs/${run_id}/${model_dir}_seed42"
