...
3. Decide whether the workflow is directly runnable today.
   If the task aligns with the current `mtsf` or maintained `grid3d_forecasting` path, produce exact experiment, benchmark, and report commands. If the task needs graph support or grid semantics outside `grid3d_forecasting`, produce the workflow-extension plan instead of stopping.
...
5. Separate current runtime advice from future task design.
   The current codebase can run sequence forecasting through `mtsf` and maintained 3D grid forecasting through `grid3d_forecasting`. Graph tasks and custom grid semantics should receive a concrete workflow expansion plan across `data`, `task`, config, and reporting layers.
