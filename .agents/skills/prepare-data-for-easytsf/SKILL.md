...
3. Map the data onto the current repository surface.
   If the data fits the current sequence path, produce concrete config fields for `task: mtsf`. If the data fits the maintained Grid3D contract, produce concrete config fields for `task: grid3d_forecasting`.
4. Produce an extension plan when the task is not yet implemented.
   For graph data or grid data outside the maintained Grid3D contract, do not stop just because the repository lacks a runnable path today. Instead, spell out:
