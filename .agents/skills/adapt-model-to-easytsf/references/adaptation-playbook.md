...
- if the model aligns with `mtsf` or maintained `grid3d_forecasting`, produce a direct adaptation plan
- if the model depends on graph inputs or grid semantics outside the maintained Grid3D contract, produce a repository extension plan
...
Use the experiment stubs and the shared benchmark stub as contract sketches; only `mtsf` and `grid3d_forecasting` are runnable tasks in the current registry.
